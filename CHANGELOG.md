...
- **커스텀 capability 6종** — `pcPower`(전원 상태), `pcExec`(명령 실행과 마지막 실행), `pcCountdown`(예약 표시·취소와 예약할 명령), `pcUser`(잠금·유휴, 옵트인), `pcInfo`(연결·업데이트·WoL·메시지), `pcVersion`(버전 줄 하나, 화면 맨 아래). 정의와 프레젠테이션 JSON은 `edge/capabilities/`에 있습니다 (#72 #86)
...
- **명령 목록을 그냥 닫아도 오류가 나지 않습니다** — 명령 목록을 열었다가 아무것도 고르지 않고 나오면 "네트워크 또는 서버 오류" 팝업이 뜨던 문제를 고쳤습니다. 앱이 목록을 닫을 때 **그 줄의 현재 값을 명령 인자로 그대로 보내기** 때문이며, 그래서 줄이 가질 수 있는 값은 모두 유효하고 아무 일도 하지 않는 인자여야 합니다. 명령 목록은 이제 늘 `명령 선택…`에 머물고, 그것을 다시 보내도 새로 고침만 합니다. 대신 **무엇이 실행됐는지는 새 `마지막 실행` 줄**("잠금 · SmartThings · 16:16")이 보여 줍니다(5초 뒤 사라지던 표시보다 오래 남습니다). 예약 줄이 분만 고를 수 있었던 것도 함께 고쳐서, **`예약할 명령` 줄**에서 종료·재시작·절전·최대 절전 중 무엇을 예약할지 고릅니다. 명령 capability는 `pcAction` → **`pcRun`**으로 이름이 바뀌었고(정의가 바뀌면 새 id가 필요합니다) 기존 장치는 새 프로필 `pc.v11`로 자동 이전됩니다 (#84)
- **예약 취소가 동작하고, 예약할 명령이 예약 카드 안으로 들어왔습니다** — 예약 목록에서 `취소`를 고르면 "시스템 오류" 팝업만 뜨던 문제를 고쳤습니다. 취소 항목은 `0분`을 보내는데 capability 정의가 최소 1분이었고, **인자 검증은 클라우드가 정의를 보고 하기 때문에 명령이 허브에 닿지도 못했습니다.** 이제 최소값이 0이고, 앞으로도 목록의 모든 항목이 정의의 인자 범위 안에 있도록 테스트가 지킵니다. 또한 `예약할 명령` 줄이 PC 명령 카드에 있어 예약 카드만 보면 시간만 고르는 화면이었던 것을 고쳐, 그 줄을 **예약 카드의 첫 줄**로 옮겼습니다(앱은 줄을 그 줄을 소유한 capability의 카드에 그립니다). 두 capability 모두 정의가 바뀌었으므로 이름이 `pcPlan` → **`pcCountdown`**, `pcRun` → **`pcExec`**으로 바뀌었고, 기존 장치는 새 프로필 `pc.v12`로 자동 이전됩니다. 이름이 바뀐 capability는 허브에서 모든 값이 비어 있는 상태로 시작하므로, 이전 직후 드라이버가 두 카드의 모든 줄을 한 번 다시 칠해 "-"와 "상태를 모두 보고하지 않았습니다" 안내가 남지 않게 합니다 (#85)
- **화면 맨 아래에 버전 줄이 생겼습니다** — "서비스 v1.1.0 · 드라이버 1.0.0 · 화면 pc.v12". 장치의 화면은 **추가한 시점의 정의로 굳기** 때문에("왜 아직 옛날 화면이지?"), 서비스·드라이버 버전만큼이나 지금 쓰는 프로필 이름이 중요합니다. PC에 아직 연결되지 않았으면 서비스 자리에 `?`가 들어가고 줄 자체는 그대로 나옵니다. 상태 capability는 속성이 늘어 `pcHealth` → **`pcInfo`**로 이름이 바뀌었고, 앱이 카드를 프로필의 capability 순서대로 그리므로 **목록 맨 끝**으로 옮겨 카드가 화면 아래에 오게 했습니다 (#85)
- **버전 줄이 잘리지 않고, 빈 줄과 회전 표시 오류가 사라졌습니다** — `상태`와 `버전`이 한 카드에 있어 앱이 두 칸으로 나란히 그리는 바람에 양쪽 다 "…"로 잘리던 것을 고쳤습니다. 값 줄이 하나뿐인 capability 만 전체 폭으로 그려지므로 버전 줄을 **새 capability `pcVersion`**(프로필 맨 끝)으로 옮겼습니다. 빈 문자열도 앱에서 "-"로 보이기 때문에 아직 실행한 명령이 없으면 `마지막 실행`이 **"없음 (None)"**이라고 적고, 모든 줄이 어떤 상황에서도 문구를 갖는지 테스트가 지킵니다. 또 값이 바뀌지 않는 명령(예약이 없을 때 `취소`, `예약할 명령`에서 같은 값 재선택, 명령 목록을 그냥 닫기)이 회전 표시 끝에 오류로 끝나던 것도 고쳤습니다 — 명령의 응답으로 나가는 이벤트를 `state_change`로 강제해 앱이 기다리던 갱신을 반드시 받게 했습니다. 기존 장치는 새 프로필 `pc.v13`으로 자동 이전됩니다 (#86)
