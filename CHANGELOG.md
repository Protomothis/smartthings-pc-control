...
- **커스텀 capability 5종** — `pcPower`(전원 상태), `pcExec`(명령 실행과 마지막 실행), `pcCountdown`(예약 표시·취소와 예약할 명령), `pcUser`(잠금·유휴, 옵트인), `pcInfo`(연결·버전·업데이트·WoL·메시지와 버전 줄, 화면 맨 아래). 정의와 프레젠테이션 JSON은 `edge/capabilities/`에 있습니다 (#72)
