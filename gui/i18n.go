...
var messages = map[string]map[Lang]string{
	...
	"status.unreachable":         {LangKo: "서비스에 연결할 수 없습니다 (포트 5002) — 서비스가 실행 중인지 확인하세요", LangEn: "Service not reachable on port 5002 — is it running?"},
	...
	"settings.remote":            {LangKo: "WebUI 브라우저 접속 허용 (로컬+LAN) — 시크릿 필요, 재시작 후 적용", LangEn: "Allow browser WebUI access (local+LAN) — requires secret, applies after restart"},
	"settings.remote.needsecret": {LangKo: "브라우저 접속을 켜려면 시크릿을 먼저 설정하세요", LangEn: "Set a secret before enabling browser access"},
	"settings.grace":             {LangKo: "원격 전원 명령 5분 유예 — SmartThings의 종료/재시작/절전/최대절전만, 취소 가능", LangEn: "5-min grace for remote power commands — SmartThings shutdown/restart/suspend/hibernate only, cancellable"},
	...
	"cmd.immediate.note":         {LangKo: "이 탭의 명령은 즉시 실행됩니다. 5분 유예는 SmartThings 원격 명령에만 적용됩니다.", LangEn: "Commands on this tab run immediately. The 5-min grace applies only to remote SmartThings commands."},
	...
	"schedule.minutes":           {LangKo: "지연 (분)", LangEn: "Delay (minutes)"},
	...
	"schedule.set":               {LangKo: "예약됨: %d분 후 %s", LangEn: "Scheduled: %s in %d minutes"},
	"schedule.invalid":           {LangKo: "분은 1~1440 사이여야 합니다", LangEn: "Minutes must be between 1 and 1440"},
