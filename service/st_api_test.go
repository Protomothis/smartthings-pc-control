...
func TestSTUnknownRouteIs404(t *testing.T) {
	stSetup(t, Config{Port: 5001})
	// /st/v1/description (#69) is not here yet; unknown paths must not
	// reach the legacy /{secret}/{command} handler. /st/v1/subscribe now
	// exists (#68) and answers 405 to a GET, which TestSTSubscribe covers.
	for _, path := range []string{"/st/v1/description", "/st/v1/"} {
