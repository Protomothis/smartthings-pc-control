...
func TestSTUnknownRouteIs404(t *testing.T) {
	stSetup(t, Config{Port: 5001})
	// /st/v1/subscribe (#68) and /st/v1/description (#69) are not here yet;
	// they must not reach the legacy /{secret}/{command} handler.
	for _, path := range []string{"/st/v1/subscribe", "/st/v1/description", "/st/v1/"} {
