...
on --(schedule.active for shutdown/restart)--> on (pcPlan 카드가 표시; powerState는 유지)
...
- 인자가 정수인 `list`에는 `"argumentType": "integer"`가 필요하다(`pcPlan.schedule`의
  프리셋 목록). 없으면 문자열 키가 그대로 나가 서비스가 거부한다.
