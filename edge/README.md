...
| 예약 요약 | `종료 · 4분 남음 · SmartThings`. 없으면 `예약 없음` |
...
**예약할 명령**에서 무엇을 예약할지 고르고, **예약 시간**에서 몇 분 뒤인지 고른다.
...
동작(Then): PC 의 pcPlanner.schedule (minutes: 30, command: shutdown)
```
...
edge/
  ...
  profiles/               pc-vN.yml — 현재는 pc-v13.yml
